...
short dtmf_mittelwert(short dtmf_freq){
	return (*(pOutFrame + dtmf_freq - 1) + *(pOutFrame + dtmf_freq) 
	+ *(pOutFrame + dtmf_freq + 1))/3;
}
...
short max_mittelwert(short dtmf_mittelwert[], short rausch_mittel){
	...
	short index = 4;
	for(i = 0; i < 4; i++){
	//Mittelwert groesser als letzter Maximalwert
		if(dtmf_mittelwert[i] > tempMax){ 
		//Mittelwert groesser als Rauschmittelwert 
			if(dtmf_mittelwert[i] > (rausch_mittel>>1)){ 
				index = i;
